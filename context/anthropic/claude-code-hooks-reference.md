...
For comprehensive session management documentation, see [Claude Code SDK Sessions](./claude-code-sdk-sessions.md).

## Session History Access

Claude Code maintains a complete session history in JSONL format that hooks can access through the `transcript_path` field.

### Session History Format

The session transcript is stored in JSONL format where each line represents a message or event in the conversation. Key entry types include:
- **Summary entries**: High-level summaries with `type: "summary"` and `leafUuid` references
- **User messages**: Complete user inputs with metadata (timestamps, working directory, git branch)
- **Assistant messages**: Full Claude responses including tool calls and reasoning
- **Tool results**: Complete tool execution results and outputs
- **System events**: Hook executions, notifications, and session state changes

Each entry includes standard fields like `uuid`, `parentUuid`, `sessionId`, `timestamp`, `gitBranch`, and `cwd`.

### Accessing Session Data in Hooks

Hooks receive the `transcript_path` field pointing to the current session JSONL file:
...
with open(input_data["transcript_path"], 'r') as f:
    session_entries = [json.loads(line) for line in f if line.strip()]

# Filter for specific types
user_messages = [entry for entry in session_entries if entry.get("type") == "user"]
summaries = [entry for entry in session_entries if entry.get("type") == "summary"]
tool_calls = [entry for entry in session_entries if "tool_use" in str(entry)]
```
...
### Example: Analyzing Recent Tool Usage
...
# Find recent tool usage (last 10 minutes)
...
print(f"Recent tool usage: {len(recent_tools)} tools used")
for tool_info in recent_tools[-5:]:  # Last 5 tools
    print(f"- {tool_info['tool']} at {tool_info['time'].strftime('%H:%M:%S')}")
```

### Example: Tracking Session Progress
...
print(f"- {stats['tool_calls']} total tool calls")
print(f"- {stats['subagent_calls']} subagent calls")
print(f"- {len(stats['files_modified'])} files modified")
print(f"- {stats['git_operations']} git operations")

if stats["files_modified"]:
    print(f"\nModified files:")
    for file_path in sorted(stats["files_modified"]):
        print(f"  - {file_path}")
```
